...
import {templates} from '../models/resumetemp.model.js';
...
export const getAllTemplates = async (req, res) => {
    try {
        ...
        const templatesList = await templates.find({}, 'name image createdBy createdAt updatedAt'); // Fetch all documents
...
export const deleteTemplate = async (req, res) => {
    try {
        ...
        const existingTemplate = await templates.findById(objectId);
        if (!existingTemplate) {
            return res.status(404).json({ success: false, message: "Template not found in database" });
        }

        // Proceed to delete
        await templates.findByIdAndDelete(objectId);
